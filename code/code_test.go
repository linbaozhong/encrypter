...
`
var encoder *chacha.ChaCha
func init()  {
	var err error
	if sourceFile,err=os.OpenFile("source.txt",os.O_CREATE|os.O_RDWR|os.O_TRUNC,0755);err!=nil{
		panic(err)
	}
	if _,err=sourceFile.WriteString(TestText);err!=nil{
		panic(err)
	}
	sourceFile.Seek(0,0)
	...
	encoder=chacha.New(key,nonce)
}

func TestEncodeFile(t *testing.T) {
	dst,err:=os.OpenFile("cipher.enc",os.O_CREATE|os.O_RDWR|os.O_TRUNC,0755)
	if err != nil {
		t.Error(err)
	}

	err=EncodeFile(encoder,sourceFile,dst)
	if err != nil {
		t.Error(err)
	}
	dst.Seek(0,0)

	newFile,err:=os.OpenFile("new.txt",os.O_CREATE|os.O_RDWR|os.O_TRUNC,0755)
	if err != nil {
		t.Error(err)
	}
	err=DecodeFile(encoder,dst,newFile)
	if err != nil {
		t.Error(err)
	}
}
